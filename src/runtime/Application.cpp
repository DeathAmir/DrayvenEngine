#include "drayven/Application.hpp"
#include <SDL3/SDL.h>
#if defined(__ANDROID__)
#include <GLES3/gl3.h>
#else
#include <SDL3/SDL_opengl.h>
#endif

namespace drayven {
Application::Application() = default;
Application::~Application(){ shutdown(); }

bool Application::init(const std::string& title, int width, int height, bool resizable) {
    shutdown();
    if (!SDL_Init(SDL_INIT_VIDEO | SDL_INIT_EVENTS | SDL_INIT_AUDIO | SDL_INIT_GAMEPAD)) return false;
#if defined(__ANDROID__)
    SDL_GL_SetAttribute(SDL_GL_CONTEXT_PROFILE_MASK, SDL_GL_CONTEXT_PROFILE_ES);
    SDL_GL_SetAttribute(SDL_GL_CONTEXT_MAJOR_VERSION, 3);
    SDL_GL_SetAttribute(SDL_GL_CONTEXT_MINOR_VERSION, 0);
#else
    SDL_GL_SetAttribute(SDL_GL_CONTEXT_PROFILE_MASK, SDL_GL_CONTEXT_PROFILE_CORE);
    SDL_GL_SetAttribute(SDL_GL_CONTEXT_MAJOR_VERSION, 3);
    SDL_GL_SetAttribute(SDL_GL_CONTEXT_MINOR_VERSION, 3);
#endif
    SDL_GL_SetAttribute(SDL_GL_DOUBLEBUFFER, 1);
    Uint64 flags = SDL_WINDOW_OPENGL | SDL_WINDOW_HIGH_PIXEL_DENSITY;
    if (resizable) flags |= SDL_WINDOW_RESIZABLE;
    m_window = SDL_CreateWindow(title.c_str(), width, height, flags);
    if (!m_window) { SDL_Quit(); return false; }
    m_gl = SDL_GL_CreateContext(m_window);
    if (!m_gl) { shutdown(); return false; }
    SDL_GL_MakeCurrent(m_window, static_cast<SDL_GLContext>(m_gl));
    SDL_GL_SetSwapInterval(1);
    m_running = true;
    m_lastTicks = SDL_GetTicksNS();
    return true;
}

void Application::shutdown() {
    if (m_gl) { SDL_GL_DestroyContext(static_cast<SDL_GLContext>(m_gl)); m_gl = nullptr; }
    if (m_window) { SDL_DestroyWindow(m_window); m_window = nullptr; }
    if (m_running) SDL_Quit();
    m_running = false;
}

bool Application::poll() {
    if (!m_running) return false;
    SDL_Event e;
    while (SDL_PollEvent(&e)) {
        if (e.type == SDL_EVENT_QUIT || e.type == SDL_EVENT_WINDOW_CLOSE_REQUESTED) m_running = false;
    }
    auto now = SDL_GetTicksNS();
    m_delta = static_cast<float>(double(now - m_lastTicks) / 1'000'000'000.0);
    m_lastTicks = now;
    return m_running;
}

void Application::beginFrame(float r, float g, float b, float a) {
    if (!m_window) return;
    int w=0,h=0;
    SDL_GetWindowSizeInPixels(m_window,&w,&h);
    glViewport(0,0,w,h);
    glClearColor(r,g,b,a);
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
}

void Application::endFrame() {
    if (m_window) SDL_GL_SwapWindow(m_window);
}
}
